...
@cocotb.test()
async def test_algo_2x2_highmem(dut):
    ...
    with open(
        r.Rlocation("coralnpu_hw/tests/cocotb/tutorial/algo_2x2_highmem_test.elf"),
        "rb",
    ) as f:
    ...
    print(f"algo_2x2_highmem inputs   = {inputs.tolist()}", flush=True)
    print(f"algo_2x2_highmem expected = {expected.tolist()}", flush=True)
    print(f"algo_2x2_highmem outputs  = {outputs.tolist()}", flush=True)
    print(
        f"algo_2x2_highmem done={int(done)} status={int(status)}",
        flush=True,
    )
