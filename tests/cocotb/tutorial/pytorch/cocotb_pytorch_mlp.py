...
def _load_model_data():
    model_path = Path(__file__).with_name("pytorch_mlp_model.py")
    spec = importlib.util.spec_from_file_location("pytorch_mlp_model", model_path)
...
class PytorchMlpTester:
    def __init__(self):
        r = runfiles.Create()
        self.elf_file = r.Rlocation(
            "coralnpu_hw/tests/cocotb/tutorial/pytorch/pytorch_mlp_test.elf"
        )
        self.fixture = None
    ...
    async def setup(self, dut):
        ...
        await self.fixture.write_word("scale_divisor", int(model.SCALE_DIVISOR))
    ...
    async def test(self):
        ...
        expected_output = model.numpy_golden()
        ...
        print(f"numpy_golden={expected_output.tolist()}", flush=True)

        assert np.array_equal(ref_output, expected_output)
        assert np.array_equal(opt_output, ref_output)
        assert int(np.argmax(opt_output)) == model.EXPECTED_CLASS


@cocotb.test()
async def test_pytorch_trained_mlp(dut):
    tester = PytorchMlpTester()
