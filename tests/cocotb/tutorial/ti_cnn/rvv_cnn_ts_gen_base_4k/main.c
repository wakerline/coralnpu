// Migrated from /home/wangyy/003_research/ara/apps/rvv_cnn_ts_gen_base_4k.
...
volatile uint32_t ti_cnn_output_count __attribute__((section(".data"))) = RVV_CNN_TS_GEN_BASE_4K_OUTPUT_CH;
volatile float ti_cnn_output_f32[RVV_CNN_TS_GEN_BASE_4K_OUTPUT_CH] __attribute__((section(".data")));
volatile float ti_cnn_golden_f32[RVV_CNN_TS_GEN_BASE_4K_OUTPUT_CH] __attribute__((section(".data")));
...
int main(void) {
  ...
  rvv_cnn_ts_gen_base_4k();
  uint64_t end = ti_cnn_read_cycle64();
  uint64_t delta = end - start;
  ti_cnn_cycles_lo = (uint32_t)delta;
  ti_cnn_cycles_hi = (uint32_t)(delta >> 32);
  ti_cnn_record_perf(delta, (uint32_t)(RVV_CNN_TS_GEN_BASE_4K_MACS), &ti_cnn_macs,
                     &ti_cnn_operations, &ti_cnn_performance,
                     &ti_cnn_utilization);

  const float *got = rvv_cnn_ts_gen_base_4k_output();
  const float *golden = rvv_cnn_ts_gen_base_4k_golden_output();
  uint32_t error = 0;
  for (uint32_t i = 0; i < RVV_CNN_TS_GEN_BASE_4K_OUTPUT_CH; ++i) {
