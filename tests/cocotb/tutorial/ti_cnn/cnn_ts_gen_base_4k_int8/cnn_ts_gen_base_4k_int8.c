...
static const int16_t input_q[CNN_TS_GEN_BASE_4K_INT8_INPUT_LEN] = {
...
static int16_t act0[CNN_TS_GEN_BASE_4K_INT8_MAX_ACT]
    __attribute__((aligned(128), section(".l2")));
static int16_t act1[CNN_TS_GEN_BASE_4K_INT8_MAX_ACT]
    __attribute__((aligned(128), section(".l2")));
static float features[CNN_TS_GEN_BASE_4K_INT8_FC_IN]
    __attribute__((aligned(128), section(".l2")));
...
static inline __attribute__((always_inline)) void linear_i8(int8_t *out, const float *in) {
  for (uint64_t o = 0; o < 2; ++o) {
    ...
    for (uint64_t i = 0; i < CNN_TS_GEN_BASE_4K_INT8_FC_IN; ++i)
...
void cnn_ts_gen_base_4k_int8(void) {
  ...
  int16_t *b = act1;
  int16_t *tmp;
  for (uint64_t i = 0; i < CNN_TS_GEN_BASE_4K_INT8_INPUT_LEN; ++i)
    a[i] = input_q[i];
  ...
  a = b;
  b = tmp;
  maxpool1d_i16(b, a, 8, 128, 3, 2, 1);
  tmp = a;
  a = b;
  b = tmp;
  conv1d_relu_i8(b, a, 8, 64, 16, 5, 2, 2, 1, conv1_weight, conv1_offset,
                 conv1_shift);
  tmp = a;
  a = b;
  b = tmp;
  conv1d_relu_i8(b, a, 16, 32, 32, 5, 2, 2, 1, conv2_weight, conv2_offset,
                 conv2_shift);
  tmp = a;
  a = b;
  b = tmp;
  avgpool1d_i16_to_f32(features, a, 32, 16, 4, 4, 0);
  linear_i8(output, features);
}

const int8_t *cnn_ts_gen_base_4k_int8_output(void) { return output; }
