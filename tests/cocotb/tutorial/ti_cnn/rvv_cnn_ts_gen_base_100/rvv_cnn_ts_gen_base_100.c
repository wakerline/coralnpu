...
#include "rvv_cnn_ts_gen_base_100.h"

static const float input[RVV_CNN_TS_GEN_BASE_100_INPUT_LEN] = {
...
static const float conv1_weight[RVV_CNN_TS_GEN_BASE_100_CONV1_OUT_CH]
    __attribute__((unused)) = {0.981742144f, 0.972327471f, -0.980774164f,
                               -1.06133318f};

static const float conv1_bias[RVV_CNN_TS_GEN_BASE_100_CONV1_OUT_CH]
    __attribute__((unused)) = {0.102102987f, -0.0557702295f, -0.0199323054f,
                               0.0523947552f};

static const float conv1_fused_weight[RVV_CNN_TS_GEN_BASE_100_CONV1_OUT_CH] = {
    0.0442979092f, 0.0438731028f, -0.0442542322f, -0.0478891948f};

static const float conv1_fused_bias[RVV_CNN_TS_GEN_BASE_100_CONV1_OUT_CH] = {
    1.02387689f, 0.857164082f, -0.940797356f, -0.944108486f};

static const float conv2_weight[RVV_CNN_TS_GEN_BASE_100_CONV2_OUT_CH *
                                RVV_CNN_TS_GEN_BASE_100_CONV1_OUT_CH *
                                RVV_CNN_TS_GEN_BASE_100_CONV2_K] = {
...
static const float conv2_bias[RVV_CNN_TS_GEN_BASE_100_CONV2_OUT_CH] = {
    -0.716420114f, -0.499633372f, 1.50315475f, 0.0143293142f};

static const float
    fc_weight[RVV_CNN_TS_GEN_BASE_100_OUTPUT_CH * RVV_CNN_TS_GEN_BASE_100_FC_IN] = {
        0.677565217f, -0.00659020245f, -1.1273849f,  0.830585301f,
        -0.96919018f, 0.136846617f,    0.690473258f, -0.579433262f};

static const float fc_bias[RVV_CNN_TS_GEN_BASE_100_OUTPUT_CH] = {-0.295130789f,
                                                            0.307057142f};

static const float golden_output[RVV_CNN_TS_GEN_BASE_100_OUTPUT_CH] = {1.920814f,
                                                                  -2.080154f};

static float
    conv1_out[RVV_CNN_TS_GEN_BASE_100_CONV1_OUT_CH * RVV_CNN_TS_GEN_BASE_100_INPUT_LEN]
    __attribute__((aligned(128), section(".l2")));
static float gap_out[RVV_CNN_TS_GEN_BASE_100_FC_IN]
    __attribute__((aligned(128), section(".l2")));
static float output[RVV_CNN_TS_GEN_BASE_100_OUTPUT_CH]
    __attribute__((aligned(128), section(".l2")));

static inline __attribute__((always_inline)) void conv1x1(float *out, const float *in) {
  for (uint64_t oc = 0; oc < RVV_CNN_TS_GEN_BASE_100_CONV1_OUT_CH; ++oc) {
    for (uint64_t t = 0; t < RVV_CNN_TS_GEN_BASE_100_INPUT_LEN; ++t) {
      float value = in[t] * conv1_fused_weight[oc] + conv1_fused_bias[oc];
      out[oc * RVV_CNN_TS_GEN_BASE_100_INPUT_LEN + t] = value < 0.0f ? 0.0f : value;
    }
  }
}

static inline __attribute__((always_inline)) void conv3x1_same_gap(float *out, const float *in) {
  for (uint64_t oc = 0; oc < RVV_CNN_TS_GEN_BASE_100_CONV2_OUT_CH; ++oc) {
    float sum = 0.0f;
    for (uint64_t t = 0; t < RVV_CNN_TS_GEN_BASE_100_INPUT_LEN; ++t) {
      float acc = conv2_bias[oc];
      for (uint64_t ic = 0; ic < RVV_CNN_TS_GEN_BASE_100_CONV1_OUT_CH; ++ic) {
        for (uint64_t k = 0; k < RVV_CNN_TS_GEN_BASE_100_CONV2_K; ++k) {
          int64_t it = (int64_t)t + (int64_t)k - 1;
          if (it >= 0 && it < RVV_CNN_TS_GEN_BASE_100_INPUT_LEN) {
            uint64_t w_idx = (oc * RVV_CNN_TS_GEN_BASE_100_CONV1_OUT_CH + ic) *
                                 RVV_CNN_TS_GEN_BASE_100_CONV2_K +
                             k;
            acc += in[ic * RVV_CNN_TS_GEN_BASE_100_INPUT_LEN + (uint64_t)it] *
                   conv2_weight[w_idx];
          }
        }
      }
    ...
    out[oc] = sum / (float)RVV_CNN_TS_GEN_BASE_100_INPUT_LEN;
  }
}

static inline __attribute__((always_inline)) void linear(float *out, const float *in) {
  for (uint64_t o = 0; o < RVV_CNN_TS_GEN_BASE_100_OUTPUT_CH; ++o) {
    float acc = fc_bias[o];
    for (uint64_t i = 0; i < RVV_CNN_TS_GEN_BASE_100_FC_IN; ++i)
      acc += in[i] * fc_weight[o * RVV_CNN_TS_GEN_BASE_100_FC_IN + i];
    out[o] = acc;
  }
}
...
const float *rvv_cnn_ts_gen_base_100_output(void) { return output; }
const float *rvv_cnn_ts_gen_base_100_golden_output(void) { return golden_output; }
