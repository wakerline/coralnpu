// Migrated from /home/wangyy/003_research/ara/apps/cnn_ts_gen_base_6k_int8.
// SPDX-License-Identifier: Apache-2.0

#include <stdint.h>
#include "../ti_cnn_perf.h"
#include "cnn_ts_gen_base_6k_int8.h"
...
volatile uint32_t ti_cnn_output_count __attribute__((section(".data"))) = CNN_TS_GEN_BASE_6K_INT8_OUTPUT_CH;
volatile int8_t ti_cnn_output_i8[CNN_TS_GEN_BASE_6K_INT8_OUTPUT_CH] __attribute__((section(".data")));
volatile int8_t ti_cnn_golden_i8[CNN_TS_GEN_BASE_6K_INT8_OUTPUT_CH] __attribute__((section(".data")));


int main(void) {
  ti_cnn_status = 0;
  uint64_t start = ti_cnn_read_cycle64();
  cnn_ts_gen_base_6k_int8();
  uint64_t end = ti_cnn_read_cycle64();
  uint64_t delta = end - start;
  ti_cnn_cycles_lo = (uint32_t)delta;
  ti_cnn_cycles_hi = (uint32_t)(delta >> 32);
  ti_cnn_record_perf(delta, (uint32_t)(CNN_TS_GEN_BASE_6K_INT8_MACS), &ti_cnn_macs,
                     &ti_cnn_operations, &ti_cnn_performance,
                     &ti_cnn_utilization);

  const int8_t *got = cnn_ts_gen_base_6k_int8_output();
  const int8_t *golden = cnn_ts_gen_base_6k_int8_golden_output();
  uint32_t error = 0;
  for (uint32_t i = 0; i < CNN_TS_GEN_BASE_6K_INT8_OUTPUT_CH; ++i) {
