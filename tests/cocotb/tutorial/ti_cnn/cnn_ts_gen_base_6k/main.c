// Migrated from /home/wangyy/003_research/ara/apps/cnn_ts_gen_base_6k.
...
volatile uint32_t ti_cnn_output_count __attribute__((section(".data"))) = CNN_TS_GEN_BASE_6K_OUTPUT_CH;
volatile float ti_cnn_output_f32[CNN_TS_GEN_BASE_6K_OUTPUT_CH] __attribute__((section(".data")));
volatile float ti_cnn_golden_f32[CNN_TS_GEN_BASE_6K_OUTPUT_CH] __attribute__((section(".data")));
...
int main(void) {
  ...
  ti_cnn_record_perf(delta, (uint32_t)(CNN_TS_GEN_BASE_6K_MACS), &ti_cnn_macs,
                     &ti_cnn_operations, &ti_cnn_performance,
                     &ti_cnn_utilization);

  const float *got = cnn_ts_gen_base_6k_output();
  const float *golden = cnn_ts_gen_base_6k_golden_output();
  uint32_t error = 0;
  for (uint32_t i = 0; i < CNN_TS_GEN_BASE_6K_OUTPUT_CH; ++i) {
