...
class RvvS1DecodeInstructionSpec extends AnyFreeSpec with ChiselSim with ParallelTestExecution {
  ...
  // 测试先压缩再译码的路径，覆盖 Dispatch 实际传入 RVV core 的内部格式。
  ...
  private def test_decode(
      dut: Tester,
      // 使用 Long 是因为 Scala 没有无符号 Int。
      cases: Seq[(Long, RvvAluOp.Type)]) = {
    // 每个 case 检查 valid 和译码出的 ALU op 是否同时匹配预期。
  ...
  private def test_decode_compressed(
      ...
      // 使用 Long 是因为 Scala 没有无符号 Int。
      cases: Seq[(Long, RvvAluOp.Type)]) = {
    // 与完整译码测试相同，但输入会先经过 RvvCompressedInstruction.from_uncompressed。
    val good = cases.map {case (inst, op) =>
      dut.io.inst.poke(inst)
      ((dut.io.out_valid.peek().litValue == 1) && (dut.io.out_op.peek().litValue == op.litValue))
    }
  ...
  "Doesn't decode float load/store" in {
    // 浮点 load/store 与 RVV load/store 复用部分 opcode，RVV decoder 不应误识别。
  ...
  "Decode VAlu ops (no mask) correctly" in {
    // 不带 mask 的常规 VAlu 指令译码用例。
    val test_cases = Seq(
      // 这些指令编码由 godbolt rv32gc clang 18.1.0 生成：
      // --target=riscv32-none-eabi -march=rv32im_zve32x -O3
      ...
      // VADC 必须带 mask，因此无 mask 形式只测试 VMADC。
  ...
  "Decode VAlu ops (with mask) correctly" in {
    ...
    val test_cases = Seq(
      ...
      // 整寄存器组搬运 VMV1R/2R/4R/8R 不允许带 mask。
  ...
  "Errata 1: Decode VAlu ops with vd=vm" in {
    // 这些编码按规范是合法的，但汇编器会拒绝生成。
    // godbolt rv32gc clang 18.1.0 无法编译：
    // --target=riscv32-none-eabi -march=rv32im_zve32x -O3
    val test_cases = Seq(
