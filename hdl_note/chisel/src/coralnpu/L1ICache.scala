// ============================================================================
// L1ICache.scala — L1 指令缓存 (8KB, 4路组相联)
//
// 4-way set-associative ICache:
//   256 槽位 → 64 sets × 4 ways
//   Cache line: 256-bit (32 bytes)
//   地址划分: Tag[31:11] | Index[10:5] | Offset[4:0]
//
// 流程: IBus 请求 → CAM 查找(valid+camaddr+matchAddr) → 命中直接返回
//       → 缺失通过 AXI Read 填充 → history 伪LRU替换策略
//   支持 IFlush 全刷新
//
// SRAM: Sram_1rw_256x256 BlackBox (256×256-bit 单端口)
// ============================================================================
...
/** L1ICache — 4路组相联 L1 指令缓存 (8KB, 256 槽)
 *  一次只能处理一个事务。地址: Tag[31:11] | Index[10:5] | Offset[4:0] */
class L1ICache(p: Parameters) extends Module {
  assert(p.axi0IdBits == 4)
  assert(p.axi0DataBits == 256 || p.axi0DataBits == 128)

  val slots    = p.l1islots                                       // 256 槽
  val slotBits = log2Ceil(slots)                                  // 8-bit 槽位索引
  val assoc    = p.l1iassoc                                       // 4 路组相联
  val sets     = slots / assoc                                    // 64 组
  val setLsb   = log2Ceil(p.fetchDataBits / 8)                   // 组索引 LSB = 5 (32B行)
  val setMsb   = log2Ceil(sets) + setLsb - 1                     // 组索引 MSB = 10
  val tagLsb   = setMsb + 1                                       // Tag LSB = 11
  val tagMsb   = 31                                               // Tag MSB = 31
  ...
  // ---- CAM (Content Addressable Memory) 查找状态 ----
  val valid   = RegInit(VecInit(Seq.fill(slots)(false.B)))       // 槽位有效位 (256-bit)
  val camaddr = Reg(Vec(slots, UInt(32.W)))                      // 各槽位的缓存行地址
  val mem     = Module(new Sram_1rw_256x256())                   // 256×256-bit SRAM

  // history(set)(way): 伪LRU替换策略 — 计数值大的最近被访问
