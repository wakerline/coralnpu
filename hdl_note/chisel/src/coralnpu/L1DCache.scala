// ============================================================================
// L1DCache.scala — L1 数据缓存 (双 bank, 每 bank 256 槽 × 4 路组相联)
//
// L1DCache: 顶层 — 双 bank(0/1) 负载均衡 + AXI 读写多路复用
//   - 地址按 bank 位 (linebit) 分配到 bank0/bank1
//   - 支持跨 bank 读写 (lineend 检测)
//   - AXI 读通道: bank0/bank1 通过 ID bit 区分路由
//   - AXI 写通道: 写地址/数据/响应三通道 bank 仲裁
//   - DFlush: clean(写回脏数据)/all(刷新全部)
//
// L1DCacheBank: 单 bank 实现 — 伪LRU + SRAM(256×288-bit, 含SEC-DED ECC)
//   - CAM 查找逻辑同 L1ICache
//   - 写回: dirty 标记 + AXI Write 通道
//   - Flush 状态机: Capture→Process→MemW→AxiReady→AxiResp→End
//   - Mem8to9/Mem9to8: SEC-DED ECC 编解码
// ============================================================================
